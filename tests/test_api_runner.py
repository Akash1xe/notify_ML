...
def test_job_api_end_to_end(client: TestClient):
    created = client.post(
        "/api/jobs", json={"source_url": "https://youtube.com/watch?v=demo"}
    )
...
def test_running_job_can_be_cancelled(tmp_path: Path):
    ...
    with TestClient(create_app(settings)) as client:
        created = client.post(
            "/api/jobs", json={"source_url": "https://youtube.com/watch?v=cancel"}
        )
        job_id = created.json()["id"]
...
def test_processor_failure_is_isolated(tmp_path: Path):
    ...
    with TestClient(create_app(settings)) as client:
        created = client.post(
            "/api/jobs", json={"source_url": "https://youtube.com/watch?v=fail"}
        )
...
def test_concurrency_limit_is_respected(tmp_path: Path):
    ...
    with TestClient(app) as client:
        ids = [
            client.post(
                "/api/jobs", json={"source_url": f"https://youtube.com/watch?v={i}"}
            ).json()["id"]
            for i in range(3)
        ]
        for job_id in ids:
